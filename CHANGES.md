...
> **修订记录**
>
> - v0.1.45-dev: **终端输出对齐原版 + 代码加固 + HTML 报表增强**——① 终端输出：废弃通用绿 `[*]`，改为原版风格阶段标签（蓝 `[INF]`、`[PortScan]`、`[Service]`、`[Web] [状态码] url [标题]`、红 `[!]`），`color.go` 新增 8 个标签函数，pipeline.go 67 处 + main.go 全量替换；② 代码加固：修复 SSH TCP 连接泄漏、MongoDB disconnect 超时、RDP panic 隔离（原版 #106）、fingerdsl 安全边界（64KB/4KB/64层含测试）、pipeline 指纹写入失败告警；③ HTML 报表：严重度分组含彩色圆点、统计卡片 hover+glow、回到顶部+页脚；`go test ./...` + `go vet ./...` 全绿（28 包）
> - v0.1.44-dev: **原版 open issues 回归审计修复**——对照 `SleepingBag945/dddd` open issues 中高相关缺陷，修复 3 类当前仍可能影响 dddd-next 的问题：`-poc` 支持逗号分隔多个关键词（覆盖 #28，精准模式和直接 fuzzy POC 都生效）；`-tf` 导入 fscan `WebTitle/InfoScan` 行时提取其中 URL 重新进入 HTTP 指纹链（覆盖 #67/#109 的 HTTPS WebTitle 漏报场景，如 C-Lodop）；gopocs worker 增加 panic 隔离，第三方协议栈解析异常不会打崩整轮扫描（覆盖 #106/#78 的 RDP/grdp panic 类问题）。同时确认 MySQL 爆破已在 nuclei 前运行且不共享原版 leveldb、target 文件行已 trim、`-sd` 保留主域名、nuclei v3/自定义模板/Host 绑定/端口阈值等已有对应实现；新增回归测试，`go test ./...` 通过
> - v0.1.43-dev: **单文件 Release + 内置基础配置**——Release 资产改为 GoReleaser `binary` 单文件，不再上传项目 zip/tar 和随包 `configs/`；基础配置、指纹、字典、legacy POC 以 `configs/bundle.zip` 编译进二进制，运行时优先读取 exe 同目录 `configs/`，否则读取当前目录 `configs/`，都不存在时释放到用户 `Downloads/dddd-next/configs`；README 降低宣传性措辞，仅保留使用者需要的单文件运行和 Downloads 释放路径说明，移除维护者发布说明；补充 `.gitattributes` 固定文本/二进制换行规则，重新生成 LF 规范的内置配置包，修复 GitHub Actions 中 `TestBundleZipMatchesBaselineConfigFiles` 因 Windows CRLF 打包导致的 stale 失败；确认不使用 UPX，降低相对原版加壳产物的误报概率；`go test ./...`、`goreleaser check`、裸 exe 无外部 configs 烟测均通过
