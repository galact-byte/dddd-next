...
> **当前状态**：**原版 recon 覆盖能力已全部对齐**——gopocs 16/17、被动指纹 / ICMP / CDN / 产品路径 / 自定义端口 / 主动子域名爆破均已补齐，OOB 盲打默认启用。剩余差异为控制/调优开关（非覆盖能力）。约为原版功能面的 90%，工程质量（依赖 / 结构 / 测试）已超越原版。
...
- 自定义 / 全端口扫描（`-p "80,443,8000-8100"` 或 `-p all`；默认精选 69 端口）
...
- 被动指纹 / ICMP 存活 / CDN 识别 / 产品路径二次指纹 / 自定义端口 / 主动子域名爆破均已补齐；OOB 盲打（interactsh）经核查默认启用。

**仅剩控制 / 调优开关（非覆盖能力，不影响"能发现什么"）**：

- severity / exclude-tags 模板过滤、自定义凭据 `-up/-upf`、按阶段开关（`-no-brute` 等）、自定义 interactsh 服务器、SYN/masscan 加速

> 诚实说明：经逐项核对原版 55 个 flag 与主流程，**会影响"能发现什么"的覆盖能力已全部复刻**，剩余差异是控制粒度与性能加速项。当前约为原版功能面的 **90%**，且工程质量（依赖 / 结构 / 测试 / 多项 bug 修复）已超越原版。
